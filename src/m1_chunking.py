...
SKIP_DOCUMENT_PREFIXES = (
    "BCTC",
    "Nghi_dinh_so_13-2023",
    "so_tay_an_toan",
)


def _should_skip(path: str) -> bool:
    return os.path.basename(path).startswith(SKIP_DOCUMENT_PREFIXES)


def _read_markdown(path: str) -> str:
    """Read UTF-8 Markdown, with a Vietnamese Windows-1258 fallback."""
...
def _ocr_pdf_text(path: str) -> str:
    """OCR a scan when explicitly enabled; return empty text on failure."""
    if os.getenv("GLMOCR_ENABLED", "0") != "1":
        return ""
    try:
        try:
            ...
            from pdf_to_markdown import pdf_to_markdown
        output = pdf_to_markdown(path)
        return output.read_text(encoding="utf-8").strip()
    except Exception as error:
        print(f"  ⚠️  GLM-OCR failed for {os.path.basename(path)}: {error}")
        return ""
...
def load_documents(data_dir: str = DATA_DIR) -> list[dict]:
    ...
    for fp in sorted(glob.glob(os.path.join(data_dir, "*.md"))):
        if _should_skip(fp):
            continue
        docs.append({"text": _read_markdown(fp), "metadata": {"source": os.path.basename(fp)}})

    for fp in sorted(glob.glob(os.path.join(data_dir, "*.pdf"))):
        if _should_skip(fp):
            print(f"  ⏭️  Bỏ qua theo cấu hình: {os.path.basename(fp)}")
            continue
        text = _extract_pdf_text(fp)
        if not text:
            text = _ocr_pdf_text(fp)
        if text:
