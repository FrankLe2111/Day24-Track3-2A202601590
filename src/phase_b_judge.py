...
def pairwise_judge(question: str, answer_a: str, answer_b: str) -> dict:
    ...
    prompt = f'''Bạn là chuyên gia đánh giá chất lượng câu trả lời RAG.
...
Đánh giá theo độ chính xác, đầy đủ và súc tích. Chỉ trả về JSON:
{{"winner": "A" hoặc "B" hoặc "tie", "reasoning": "giải thích ngắn gọn", "scores": {{"A": 0.0, "B": 0.0}}}}
Điểm phải nằm trong khoảng 0 đến 1.'''
    ...
    return {
        ...
        "scores": {side: max(0.0, min(1.0, float(scores.get(side, 0.0))))
                   for side in ("A", "B")},
    }
...
def swap_and_average(question: str, answer_a: str, answer_b: str) -> JudgeResult:
    ...
    """
    # Run the two judge passes and convert the swapped result back to A/B.
...
def cohen_kappa(judge_labels: list[int], human_labels: list[int]) -> float:
    ...
    if any(label not in (0, 1) for label in [*judge_labels, *human_labels]):
        raise ValueError("Nhãn Cohen's κ chỉ được là 0 hoặc 1")
    n = len(judge_labels)
    observed = sum(judge == human for judge, human in zip(judge_labels, human_labels)) / n
    expected = sum(
        judge_labels.count(label) * human_labels.count(label) / n**2
        for label in (0, 1)
    )
...
if __name__ == "__main__":
    ...
    # This demo has one known-good reference answer (A); full κ needs 10 aligned pairs.
    judge_labels = [1 if result.final_winner == "A" else 0]
    kappa = cohen_kappa(judge_labels, [1])
    print(f"Cohen's κ (demo): {kappa:.3f}")
    ...
    with open("reports/judge_results.json", "w", encoding="utf-8") as f:
        json.dump({"kappa": kappa, "kappa_scope": "demo pair only",
                   "pairs_judged": 1, "results": [result.__dict__],
                   "bias_report": bias},
